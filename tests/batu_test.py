...
if __name__=="__main__":
    df = pd.read_excel("tests/test_data/Day_Count.xlsx",header=[0])
    f_act360 = lambda x: get_ql_dayCount(x['date1_ddh'],x['date2_ddh'],count_convention="Act360")
    f_act365 = lambda x: get_ql_dayCount(x['date1_ddh'],x['date2_ddh'],count_convention="Act365")
    f_30E360 = lambda x: get_ql_dayCount(x['date1_ddh'],x['date2_ddh'],count_convention="30E360")
    f_bd252 = lambda x: get_ql_dayCount(x['date1_ddh'],x['date2_ddh'],count_convention="bd252",cals="BR")
    disent_act360 = lambda x: (x['date2_ddh']-x['date1_ddh']).yf('ACT/360')
    disent_act365 = lambda x: (x['date2_ddh']-x['date1_ddh']).yf('ACT/365')
    disent_30e360 = lambda x: (x['date2_ddh']-x['date1_ddh']).yf('30E/360')
    disent_bd252 = lambda x: (x['date2_ddh']-x['date1_ddh']).yf('BD/252',cals="NY")
