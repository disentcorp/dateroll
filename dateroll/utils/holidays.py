...
import datetime
import numpy as np
from workalendar import usa,europe,oceania,america,asia
...
weekend_u_holiday_mapping = {
							...
							'WE_u_HOL':lambda dates,hol_list:eval(f'{dates.weekday()} in [5,6] or {dates in hol_list}'),  # in {hol_list}
...
f_dates_in_year = lambda bgn_yr,end_yr: [bgn_yr + timedelta(days=i) for i in range(int((end_yr - bgn_yr).days) + 1)]
...
def get_hol_list(yrs,cal='WE',dic={}):
	...
	for ca in calendars:
		if ca in holiday_mapping.keys():
			...
			raise Exception(f'{ca} holidays are not supported yet, please contact Disent')
...
def date_adjust(d,sgn,n,cal='WE',roll_conv=None):
	...
	if yr in dic.keys():
		...
		dic_rs = {k:v for k,v in dic.items() if list(v.keys())[0]=='r' and list(v.values())[0]==val}
		rs = list(dic_rs.keys())[0]
		rs = date_conv_adjust(dic,yr,rs,roll_conv)

		return rs
	else:
		raise Exception(f'{yr} year is not within the year range.Asian holidays between 2000-2050, Other countries between 1990-2080')
